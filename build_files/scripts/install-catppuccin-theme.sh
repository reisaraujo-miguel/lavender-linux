...
EOF
    ...
    (
        ...
        #/usr/bin/kpackagetool6 -g -i "$GLOBALTHEMENAME".tar.gz -p /usr/local/share/plasma/look-and-feel/
...
if [ "$CONFIRMATION" = "Y" ] || [ "$CONFIRMATION" = "y" ]; then
    ...
    if [ "$CONFIRMATION" = "Y" ] || [ "$CONFIRMATION" = "y" ]; then
		...
		echo "LookAndFeelPackage=$GLOBALTHEMENAME" >> /etc/skel/.config/kdeglobals
